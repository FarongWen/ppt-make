...
def format_date(date_value):
    ...
    if isinstance(date_value, pd.Timestamp):
        ...
        try:
            ...
            # Handle period-separated dates like '2025.12.26-12.30'
            elif '.' in date_value:
                # Only take the first part if there's a range
                if '-' in date_value:
                    date_value = date_value.split('-')[0]
                # Parse with period separator
                try:
...
def main():
    try:
        # Read the Excel file
        excel_file = "2026年3-4月待转正党员信息.xlsx"
        ...
        # Extract the template 2 part (after "# template 2")
        template_parts = template_content.split("# template 2")
        if len(template_parts) < 2:
            raise Exception("Template 2 not found in requirement.md")
        template = template_parts[1].strip()
        ...
        output_file = "2026年3-4月待转正党员信息-整理后.xlsx"
        
        # Create a DataFrame with the text content in the first column only
        df_output = pd.DataFrame({
            "党员转正信息": [item["text"] for item in results]
        })
